          ...
          <div className="flex items-center justify-between">
            ...
            {result.success && result.validation && (
              <div className="flex items-center gap-3">
                ...
                {result.validation.n8nValidation && !result.validation.n8nValidation.success && (
                  <RetryValidation
          ...
          {result.success && result.workflow && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => navigator.clipboard.writeText(result.copyableJSON || JSON.stringify(result.workflow, null, 2))}
              ...
              <pre className="p-4 bg-gray-100 rounded-lg overflow-x-auto text-sm">
                {result.copyableJSON || JSON.stringify(result.workflow, null, 2)}
              </pre>
            </div>
          )}
          ...
          {result.metadata && (
            <details className="text-sm">
              ...
              <div className="mt-2 p-3 bg-gray-50 rounded space-y-1">
                ...
                <p>Generation Time: {(result.metadata.generationTime / 1000).toFixed(2)}s</p>
                {result.metadata.timing?.n8nValidation > 0 && (
                  <p>Validation Time: {(result.metadata.timing.n8nValidation / 1000).toFixed(2)}s</p>
                )}
                <p>Tokens Used: {result.metadata.inputTokens + result.metadata.outputTokens}</p>
