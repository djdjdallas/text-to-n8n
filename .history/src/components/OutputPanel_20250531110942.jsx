        ...
        return output ? (
          ...
          <div className="flex h-full items-center justify-center text-muted">
            {isGenerating ? (
              ...
              <p>Enter your workflow description and click "Generate Workflow"</p>
            )}
          </div>
        );
      case 'visual':
      ...
      <div className="border-b border-border">
        <nav className="flex">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              className={`px-4 py-2 text-sm font-medium ${activeTab === tab.id ? 'border-b-2 border-primary text-primary' : 'text-muted hover:text-foreground'}`}
          ...
          <div className="rounded-md bg-card/50 p-4">
            ...
            <ul className="space-y-2 text-sm">
              <li className="flex items-start">
                <svg className="mr-2 h-5 w-5 text-primary" fill="none" viewBox="0 0 24 24">
                  <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12.5l3 3 5-6"></path>
                </svg>
