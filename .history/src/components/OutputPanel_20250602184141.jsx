...
const OutputPanel = ({ output, isGenerating, generationTime }) => {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {output && output._metadata && (
        <div className="border-t border-border p-4">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-muted">Model:</span>
              <p className="font-medium">{output._metadata.model}</p>
            </div>
            <div>
              <span className="text-muted">Validation Score:</span>
              <p className="font-medium">
                {output._metadata.validation?.score}/100
              </p>
            </div>
            <div>
              <span className="text-muted">RAG Docs Used:</span>
              <p className="font-medium">{output._metadata.ragDocs || 0}</p>
            </div>
            <div>
              <span className="text-muted">Valid:</span>
              <p className="font-medium">
                {output._metadata.validation?.isValid ? "✅ Yes" : "❌ No"}
              </p>
            </div>
          </div>

          {output._metadata.instructions && (
            <div className="mt-4 rounded-md bg-blue-500/10 p-3">
              <h4 className="font-medium mb-2">Import Instructions:</h4>
              <ol className="list-decimal list-inside text-sm space-y-1">
                {output._metadata.instructions.map((instruction, i) => (
